...
String product_note=new String(request.getParameter("product_note").getBytes("ISO-8859-1"),"utf-8");
String product_price=request.getParameter("product_price");
String product_left=request.getParameter("product_left");
String product_rightnote=new String(request.getParameter("product_rightnote").getBytes("ISO-8859-1"),"utf-8");
String product_downnote=new String(request.getParameter("product_downnote").getBytes("ISO-8859-1"),"utf-8");
...
try{
	...
	try{
		...
		else
		{
			...
			if(request.getParameter("hide").equals("ON"))
			{
			sql="SELECT* FROM product WHERE product_id='"+which+"'";
			ResultSet rs=con.createStatement().executeQuery(sql);
			    if(rs.next())
			   {
				sql="UPDATE product SET product_name='"+product_name+"',product_note='"+product_note+"',product_price='"+product_price+"',product_left='"+product_left+"',product_rightnote='"+product_rightnote+"',product_downnote='"+product_downnote+"' WHERE product_id='"+which+"'";;
				int no=con.createStatement().executeUpdate(sql);
				
				if(no>0)
				out.println("<br><br><br><div style='text-align:center;'><h1>修改成功!! <a href='administrator.jsp'>點我</a>回管理員專區</h1></div>");

			   }
			}	
			else
			{
				sql="SELECT* FROM wait_product WHERE product_id='"+which+"'";
			    ResultSet rs=con.createStatement().executeQuery(sql);
			    if(rs.next())
			   {
				sql="UPDATE wait_product SET product_name='"+product_name+"',product_note='"+product_note+"',product_price='"+product_price+"',product_left='"+product_left+"',product_rightnote='"+product_rightnote+"',product_downnote='"+product_downnote+"' WHERE product_id='"+which+"'";;
				int no=con.createStatement().executeUpdate(sql);
