...
try{
	...
	try{
		...
		else
		{
			...
			sql="SELECT* FROM wait_product WHERE product_id='"+choose+"'";
			ResultSet rs=con.createStatement().executeQuery(sql);
			if(rs.next())
			{
				sql="INSERT product(product_id,product_name,product_note,product_price,product_left,product_rightnote,product_downnote)"+
				    "VALUES('"+rs.getString(1)+"','"+rs.getString(2)+"','"+rs.getString(3)+"','"+rs.getString(4)+"','"+rs.getString(5)+"','"+rs.getString(6)+"','"+rs.getString(7)+"')";
			    con.createStatement().execute(sql);
				
				sql="DELETE FROM wait_product WHERE product_id='"+choose+"'";
				int no=con.createStatement().executeUpdate(sql);
				out.println("<br><br><br><div style='text-align:center;'><h1>上架成功!! <a href='administrator.jsp'>點我回管理員專區</a></h1></div>");
